...
class Admin_Controller extends Site_Controller
{
    ...
    public function facebook_logout()
 {
  ...
  {  
   $this->facebook->getLogoutUrl(array('access_token'=>$this->facebook->getAccessToken()));
   ...
   setcookie('fbm_'.$this->facebook->getAppId(),'',time()-3600*24,'/',$_SERVER['HTTP_HOST']);
   setcookie('fbsr_'.$this->facebook->getAppId(),'',time()-3600*24,'/',$_SERVER['HTTP_HOST']);  
  }
